...
const createOrder = async (req, res) => {
    ...
    try {
        await pool.query('INSERT INTO orders (id, merchant_id, amount, currency, receipt, notes, status) VALUES ($1, $2, $3, $4, $5, $6, $7)', 
            [orderId, req.merchant.id, amount, currency, receipt, notes || {}, 'created']);
        res.status(201).json({ 
            id: orderId, merchant_id: req.merchant.id, amount, currency, receipt, 
            notes: notes||{}, status: "created", created_at: new Date().toISOString() 
        });
